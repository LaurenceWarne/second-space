...
public class TemplateSystem extends IteratingSystem {
    ...
    @NonNull
    private final Map<String, InputStream> entityNameToStreamMap = new HashMap<>();
	...
	entityNameToStreamMap.put(template.getName(), template.getStream());
    }
	...
	final EntityTemplate template = mEntityTemplate.get(id);
	entityNameToStreamMap.remove(template.getName());
    }
	...
	if (entityNameToStreamMap.containsKey(name)){
	    final InputStream is = entityNameToStreamMap.get(name);
